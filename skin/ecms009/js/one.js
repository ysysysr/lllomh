...
!function(b,e,g,a){var c=b(e);b.fn.lazyload=function(d){function f(){var a=0;j.each(function(){var d=b(this);if((!h.skip_invisible||d.is(":visible"))&&!b.abovethetop(this,h)&&!b.leftofbegin(this,h))if(b.belowthefold(this,h)||b.rightoffold(this,h)){if(++a>h.failure_limit)return!1}else d.trigger("appear"),a=0})}var k,j=this,h={threshold:0,failure_limit:0,event:"scroll",effect:"show",container:e,data_attribute:"original",skip_invisible:!0,appear:null,load:null,placeholder:"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC"};return d&&
(a!==d.failurelimit&&(d.failure_limit=d.failurelimit,delete d.failurelimit),a!==d.effectspeed&&(d.effect_speed=d.effectspeed,delete d.effectspeed),b.extend(h,d)),k=h.container===a||h.container===e?c:b(h.container),0===h.event.indexOf("scroll")&&k.bind(h.event,function(){return f()}),this.each(function(){var d=this,c=b(d);d.loaded=!1;(c.attr("src")===a||!1===c.attr("src"))&&c.attr("src",h.placeholder);c.one("appear",function(){this.loaded||(h.appear&&h.appear.call(d,j.length,h),b("<img />").bind("load",
function(){var a=c.data(h.data_attribute);c.hide();c.is("img")?c.attr("src",a):c.css("background-image","url('"+a+"')");c[h.effect](h.effect_speed);d.loaded=!0;a=b.grep(j,function(a){return!a.loaded});(j=b(a),h.load)&&h.load.call(d,j.length,h)}).attr("src",c.data(h.data_attribute)))});0!==h.event.indexOf("scroll")&&c.bind(h.event,function(){d.loaded||c.trigger("appear")})}),c.bind("resize",function(){f()}),/iphone|ipod|ipad.*os 5/gi.test(navigator.appVersion)&&c.bind("pageshow",function(a){a.originalEvent&&
a.originalEvent.persisted&&j.each(function(){b(this).trigger("appear")})}),b(g).ready(function(){f()}),this};b.belowthefold=function(d,f){var g;return g=f.container===a||f.container===e?(e.innerHeight?e.innerHeight:c.height())+c.scrollTop():b(f.container).offset().top+b(f.container).height(),g<=b(d).offset().top-f.threshold};b.rightoffold=function(d,f){var g;return g=f.container===a||f.container===e?c.width()+c.scrollLeft():b(f.container).offset().left+b(f.container).width(),g<=b(d).offset().left-
f.threshold};b.abovethetop=function(d,f){var g;return g=f.container===a||f.container===e?c.scrollTop():b(f.container).offset().top,g>=b(d).offset().top+f.threshold+b(d).height()};b.leftofbegin=function(d,f){var g;return g=f.container===a||f.container===e?c.scrollLeft():b(f.container).offset().left,g>=b(d).offset().left+f.threshold+b(d).width()};b.inviewport=function(a,c){return!(b.rightoffold(a,c)||b.leftofbegin(a,c)||b.belowthefold(a,c)||b.abovethetop(a,c))};b.extend(b.expr[":"],{"below-the-fold":function(a){return b.belowthefold(a,
...
document);
!function(){var b=jQuery.event.special,e="D"+ +new Date,g="D"+(+new Date+1);b.scrollstart={setup:function(){var a,c=function(c){var e=arguments;a?clearTimeout(a):(c.type="scrollstart",jQuery.event.dispatch.apply(this,e));a=setTimeout(function(){a=null},b.scrollstop.latency)};jQuery(this).bind("scroll",c).data(e,c)},teardown:function(){jQuery(this).unbind("scroll",jQuery(this).data(e))}};b.scrollstop={latency:300,setup:function(){var a,c=function(c){var e=this,g=arguments;a&&clearTimeout(a);a=setTimeout(function(){a=
null;c.type="scrollstop";jQuery.event.dispatch.apply(e,g)},b.scrollstop.latency)};jQuery(this).bind("scroll",c).data(g,c)},teardown:function(){jQuery(this).unbind("scroll",jQuery(this).data(g))}}}();
jQuery.cookie=function(b,e,g){if("undefined"!=typeof e){g=g||{};null===e&&(e="",g.expires=-1);var a="";if(g.expires&&("number"==typeof g.expires||g.expires.toUTCString))"number"==typeof g.expires?(a=new Date,a.setTime(a.getTime()+864E5*g.expires)):a=g.expires,a="; expires="+a.toUTCString();var c=g.path?"; path="+g.path:"",d=g.domain?"; domain="+g.domain:"",g=g.secure?"; secure":"";document.cookie=[b,"=",encodeURIComponent(e),a,c,d,g].join("")}else{e=null;if(document.cookie&&""!=document.cookie){g=
document.cookie.split(";");for(a=0;a<g.length;a++)if(c=jQuery.trim(g[a]),c.substring(0,b.length+1)==b+"="){e=decodeURIComponent(c.substring(b.length+1));break}}return e}};var LS={get:function(b){return window.localStorage?localStorage.getItem(b):$.cookie(b)},set:function(b,e){window.localStorage?localStorage[b]=e:$.cookie(b,e)},remove:function(b){window.localStorage?localStorage.removeItem(b):$.cookie(b,void 0)}};
                    ...
                    <div class="modal-dialog" style="margin-top:200px;width:340px;">\
                        <div class="modal-content">\
                            <div class="modal-header">\
                                ...
                                <h4 class="modal-title">微信扫一扫捐赠作者</h4>\
                            </div>\
    ...
    */
    ...
    window._bd_share_config = {
        common: {
            ...
            "bdMiniList": false,
            "bdPic": $('.article-pic img:first') ? $('.article-pic img:first').attr('src') : '',
            "bdStyle": "0",
            "bdSize": "24"
        },
        share: [{
        ...
        switch (type) {
            ...
            case 'login':
                ...
                if( !window._loginCache ){
                    $.ajax({  
                        type: 'POST',  
                        url:  jsui.uri+'/action/mo-login.php',  
